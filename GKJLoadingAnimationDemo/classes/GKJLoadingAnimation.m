...
@implementation GKJLoadingAnimation
{
    CADisplayLink *_link;
...
+(GKJLoadingAnimation*)showIn:(UIView*)view{
    [self hideIn:view];
    GKJLoadingAnimation *hud = [[GKJLoadingAnimation alloc] initWithFrame:view.bounds];
    [hud start];
    [view addSubview:hud];
    return hud;
}
...
+(GKJLoadingAnimation *)hideIn:(UIView *)view{
    GKJLoadingAnimation *hud = nil;
    for (GKJLoadingAnimation *subView in view.subviews) {
        if ([subView isKindOfClass:[GKJLoadingAnimation class]]) {
            [subView hide];
