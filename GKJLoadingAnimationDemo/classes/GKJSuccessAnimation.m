...
@implementation GKJSuccessAnimation
{
...
+(GKJSuccessAnimation*)showIn:(UIView*)view{
    [self hideIn:view];
    GKJSuccessAnimation *hud = [[GKJSuccessAnimation alloc] initWithFrame:view.bounds];
    [hud start];
    [view addSubview:hud];
    return hud;
}
...
+(GKJSuccessAnimation *)hideIn:(UIView *)view{
    GKJSuccessAnimation *hud = nil;
    for (GKJSuccessAnimation *subView in view.subviews) {
        if ([subView isKindOfClass:[GKJSuccessAnimation class]]) {
            [subView hide];
